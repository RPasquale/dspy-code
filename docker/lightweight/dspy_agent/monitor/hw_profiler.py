...
def _nvidia_smi_query() -> Dict[str, Any]:
    if not shutil.which('nvidia-smi'):
        return {}
    try:
        q = ['utilization.gpu', 'memory.used', 'memory.total', 'power.draw', 'name']
...
def read_hw() -> HWSnapshot:
    ...
    if psutil is not None:
        try: snap.cpu_percent = float(psutil.cpu_percent(interval=None))
...
class HardwareProfiler(threading.Thread):
    ...
    def run(self) -> None:  # pragma: no cover
        while not self._stop.is_set():
            snap = read_hw(); data = asdict(snap)
            try: self._out.write_text(json.dumps(data, indent=2))
            except Exception: pass
            if self._storage is not None:
                try: self._storage.append('hw_metrics', data)  # type: ignore[attr-defined]
                except Exception: pass
