...
const (
	...
	EnvManagerService_StreamLogs_FullMethodName              = "/env_manager.v1.EnvManagerService/StreamLogs"
)
...
func (c *envManagerServiceClient) StreamLogs(ctx context.Context, in *StreamLogsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[LogEntry], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &EnvManagerService_ServiceDesc.Streams[3], EnvManagerService_StreamLogs_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EnvManagerService_StreamLogsClient = grpc.ServerStreamingClient[LogEntry]
...
func (UnimplementedEnvManagerServiceServer) StreamLogs(*StreamLogsRequest, grpc.ServerStreamingServer[LogEntry]) error {
...
func _EnvManagerService_StreamLogs_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamLogsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(EnvManagerServiceServer).StreamLogs(m, &grpc.GenericServerStream[StreamLogsRequest, LogEntry]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EnvManagerService_StreamLogsServer = grpc.ServerStreamingServer[LogEntry]
...
var EnvManagerService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _EnvManagerService_StreamLogs_Handler,
			ServerStreams: true,
		},
	},
